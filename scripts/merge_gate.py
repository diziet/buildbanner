...
# A *.md under these directories sits beside code or tests, so its PR runs the full gate.
CODE_DIRS = (
    "client/",
    "examples/",
    "node/",
...
def temp_worktree(repo: Path, base_sha: str) -> Iterator[Path]:
    ...
    The tree is on a branch, not a detached HEAD, so code that reads the branch name
    gets one, as it does in a normal worktree. In mathviz and writing-generator, a
    detached preview failed a test that expects buildbanner to report a branch.
    """
    tree = Path(tempfile.mkdtemp(prefix="buildbanner-merge-"))
...
def is_docs_only(paths: list[str]) -> bool:
    """True when every changed path is a *.md outside CODE_DIRS."""
    if not paths:
        return False
    return all(
        Path(p).suffix in DOCS_SUFFIXES and not p.startswith(CODE_DIRS) for p in paths
    )
