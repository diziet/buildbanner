...
def _fake_gh(
    ...
    """Point the session's fake gh at a `gh pr list` answer naming `open_branches`."""
    listing = tmp_path / "open-prs.json"
    listing.write_text(json.dumps([{"headRefName": b} for b in open_branches]))
    monkeypatch.setenv("FAKE_GH_LIST_JSON", str(listing))
...
def test_gather_reads_git_state(
    ...
    _fake_gh(fake_gh_dir, tmp_path, monkeypatch, ["feat/open"])
...
def test_delete_removes_only_the_merged_class(
    ...
    git_repo.git("switch", "-q", "main")
    _fake_gh(fake_gh_dir, tmp_path, monkeypatch, [])
