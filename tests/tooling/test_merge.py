...
def test_gate_failure_refuses_and_removes_temp_worktree(
    ...
    assert "buildbanner-merge-" not in worktrees
    assert pr.repo.git("branch", "--list", "merge-preview/*").stdout == ""
...
def test_docs_only_pr_skips_heavy_stages(
    pr: MergeFixture, capsys: pytest.CaptureFixture[str]
) -> None:
    pr.repo.git("switch", "-q", "-c", "docs/only", "origin/main")
    pr.head_sha = pr.repo.commit_file("NOTES.md", "notes\n", "docs")
    pr.repo.git("push", "-q", "-u", "origin", "docs/only")
    pr.repo.git("switch", "-q", "main")
    pr.set_pr(headRefName="docs/only")
    assert pr.run("--dry-run") == 0
    assert "docs-only PR" in capsys.readouterr().out
...
@pytest.mark.parametrize(
    ...
    [
        (["CLAUDE.md"], True),
        (["CLAUDE.md", "docs/README.md"], True),
        (["python/README.md"], False),
        (["tests/parity/README.md"], False),
        (["CLAUDE.md", "scripts/x.py"], False),
        ([], False),
    ],
)
def test_is_docs_only(paths: list[str], expected: bool) -> None:
    assert merge_gate.is_docs_only(paths) is expected


@pytest.mark.parametrize(
    ("url", "owner"),
    [
        ("https://github.com/diziet/buildbanner.git", "diziet"),
        ("git@github.com:diziet/buildbanner.git", "diziet"),
        ("/tmp/origin.git", "tmp"),
    ],
)
...
def test_default_gate_commands_install_deps_then_run_gate(tmp_path: Path) -> None:
    commands = merge.default_gate_commands(tmp_path, docs_only=True)
    assert commands[0] == ["make", "-C", str(tmp_path), "-s", "deps"]
    assert commands[1][-1] == "--docs-only"
