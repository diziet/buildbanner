"""scripts/merge_gate: the preview worktree and the message for a failed merge."""
...
def test_preview_tree_is_on_a_throwaway_branch_removed_on_exit(
    ...
    """Code that reads the branch name gets one, as it does in a normal worktree."""
    with merge_gate.temp_worktree(git_repo.clone, git_repo.head()) as tree:
        head = git_repo.git(
            "symbolic-ref", "--quiet", "--short", "HEAD", cwd=tree, check=False
        )
        assert head.stdout.strip() == f"merge-preview/{tree.name}"
